    ...
    final class UserService {
        ...
        func getUserName(firUid: String, completion: @escaping (Result<User, FirestoreError>) -> Void) {
        ...
        func updateActiveBath(firUid: String, isActiveBath: Bool, completion: @escaping (Result<Bool, FirestoreError>) -> Void) {
            db.collection("user").whereField("firebaseUid", isEqualTo: firUid).getDocuments { snapshot, error in
                if error != nil {
                    completion(.failure(.other))
                    return
                ...
                if let deviceRef = documetReF {
                    deviceRef.updateData(["activeBath": isActiveBath]) { error in
                        if error != nil {
                            completion(.failure(.writingFailed))
                            return
                        }
